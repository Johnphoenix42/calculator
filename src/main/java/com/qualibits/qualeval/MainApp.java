...
public class MainApp extends Application {
    ...
    public MainApp() {
        ...
        appSetting.setParser(expressionParser);
    ...
    LinkedList<BaseButton> getCalcButtons(int col, int row) {
        ...
        buttonList.add(new TermButton<>(ButtonName.OPEN_PARENTHESIS, event -> expressionScreen.setText(expressionParser.printExpressionQueue(openParenthesis)), openParenthesis, col, row - 2));
        buttonList.add(new TermButton<>(ButtonName.CLOSE_PARENTHESIS, event -> expressionScreen.setText(expressionParser.printExpressionQueue(closeParenthesis)), closeParenthesis, 1 + col, row - 2));
        ...
        buttonList.add(new TermButton<>(ButtonName.HYPOTENUSE, event -> expressionScreen.setText(expressionParser.printExpressionQueue(hypotenuseOperator)), hypotenuseOperator, col, row - 1));
        buttonList.add(new TermButton<>(ButtonName.FACTORIAL, event -> expressionScreen.setText(expressionParser.printExpressionQueue(factorialOperator)), factorialOperator, col + 1, row - 1));
        ...
        buttonList.add(new TermButton<>(ButtonName.X_POWER_Y, event -> expressionScreen.setText(expressionParser.printExpressionQueue(xPowerY)), xPowerY, col, row));
        buttonList.add(new TermButton<>(ButtonName.INVERSE, event -> expressionScreen.setText(expressionParser.printExpressionQueue(inverseOperator)), inverseOperator, col + 1, row));

        buttonList.add(new TermButton<>(ButtonName.SQUARE, event -> expressionScreen.setText(expressionParser.printExpressionQueue(squareOperator)), squareOperator, col, row + 1));
        buttonList.add(new TermButton<>(ButtonName.PI, event -> {
            expressionScreen.setText(expressionParser.printExpressionQueue(piOperand));
            computeScreen.setText(piOperand.toString());
        }, piOperand, col + 1, row + 1));
        buttonList.add(new TermButton<>(ButtonName.EULER, event -> {
            expressionScreen.setText(expressionParser.printExpressionQueue(eulerOperand));
            computeScreen.setText(eulerOperand.toString());
        }, eulerOperand, 2 + col, row + 1));
        buttonList.add(new TermButton<>(ButtonName.MODULO, event -> expressionScreen.setText(expressionParser.printExpressionQueue(modulusOperator)), modulusOperator, 3 + col, row + 1));
        buttonList.add(new TermButton<>(ButtonName.DIVISION, event -> expressionScreen.setText(expressionParser.printExpressionQueue(divisionOperator)), divisionOperator, 4 + col, row + 1));

        buttonList.add(new TermButton<>(ButtonName.SQUARE_ROOT, event -> expressionScreen.setText(expressionParser.printExpressionQueue(rootOperator)), rootOperator, col, row + 2));
        buttonList.add(new TermButton<>(ButtonName.TEN_POWER_X, event -> expressionScreen.setText(expressionParser.printExpressionQueue(tenPowerX)), tenPowerX, col, row + 3));

        buttonList.add(new TermButton<>(ButtonName.LOG, event -> expressionScreen.setText(expressionParser.printExpressionQueue(logarithmicOperator)), logarithmicOperator, col, row + 4));
        buttonList.add(new TermButton<>(ButtonName.LN, event -> expressionScreen.setText(expressionParser.printExpressionQueue(naturalLogOperator)), naturalLogOperator, col, row + 5));
        ...
        buttonList.add(new TermButton<>(ButtonName.MULTIPLICATION, event -> expressionScreen.setText(expressionParser.printExpressionQueue(multiplicationOperator)), multiplicationOperator, 4 + col, row + 2));
        buttonList.add(new TermButton<>(ButtonName.SUBTRACTION, event -> expressionScreen.setText(expressionParser.printExpressionQueue(subtractionOperator)), subtractionOperator, 4 + col, row + 3));
        buttonList.add(new TermButton<>(ButtonName.ADDITION, event -> expressionScreen.setText(expressionParser.printExpressionQueue(additionOperator)), additionOperator, 4 + col, row + 4));
    ...
    private void handleTrigButtonsPressed(TrigOperator trigOperator){
        ...
        expressionScreen.setText(expressionParser.printExpressionQueue(trigOperator));
    }
    ...
    private void handleRecallLastAnswer(ActionEvent event) {
        ...
        expressionParser.printExpressionQueue(lastAnsTerm);
    }
    ...
    private void handleEqualsPressed(ActionEvent event){
        expressionScreen.setText(expressionParser.printExpressionQueue(null));
